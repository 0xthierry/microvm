...
print_step "create VM with options"
...
print_step "start VM with cgroup profile overrides"
run_cli_profile start "$VM_ID" --no-attach
load_status
...
EXPECTED_CGROUP_REL="/$EXPECT_CGROUP_PARENT/$VM_ID"
...
print_step "validate resource limits (rlimits)"
...
print_step "restart VM with same cgroup profile and verify limits persist"
run_cli stop "$VM_ID"
run_cli_profile start "$VM_ID" --no-attach
load_status
